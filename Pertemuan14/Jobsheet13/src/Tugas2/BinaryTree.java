...
public class BinaryTree {
    ...
    // JAWABAN NO 2
    // menampilkan data yang memiliki nilai paling kecil dan paling besar
    public int findMax(){
        return nilaiMaks(this.root);
    }
    
    int nilaiMaks(Node node){ // fungsi untuk membantu menemukan nilai maksimum
        if (node.right != null) { // karena node yg nilai nya lebih besar ada di kanan
            return nilaiMaks(node.right); // jadi jika tidak sama dengan null, maka akan me return node sebelah kanan
        }
        return node.data;
    }
    ...
    int nilaiMin(Node node){ // fungsi untuk membantu menemukan nilai minimun
